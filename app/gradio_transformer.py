...
from src.models.Transformer.generate_samples import TransformerAdapter


DEFAULT_CHECKPOINT = "experiments/transformer-tok14-batch16-sched-decay8000/gpt2"
DEFAULT_MODEL_CFG = "configs/model/transformer/tok_14_gpt2_match.yaml"
DEFAULT_TOKENIZER_CFG = "configs/data/14.yaml"
DEFAULT_OUTPUT_DIR = "experiments/gradio_transformer_generations"
DEFAULT_SOUNDFONT = ""
MIDI_PLAYER_HEAD = """
<style>
  .midi-web-player {
...
_MODEL_CACHE: LoadedTransformer | None = None
STEP_CHECKPOINT_RE = re.compile(r"^checkpoint-(\d+)$")
...
def _is_checkpoint_variant_dir(path: Path) -> bool:
    return STEP_CHECKPOINT_RE.match(path.name) is not None
...
def discover_runs(root: Path = Path("experiments")) -> list[RunSelection]:
    ...
    runs = {
        ...
        for path in root.glob("**/config.json")
        if path.is_file() and (path.parent / "model.safetensors").is_file()
    }
    return [
...
def _checkpoint_label(path: Path, run_dir: Path) -> str:
    try:
        checkpoint_dir = path.relative_to(run_dir)
    except ValueError:
        return _display_path(path)
    if str(checkpoint_dir) == ".":
        return "selected model (run root)"
...
def list_run_checkpoints(run_dir: Path | None) -> list[CheckpointSelection]:
    ...
    candidates = [run_dir, *sorted(run_dir.glob("checkpoint-*"))]
    for checkpoint_path in candidates:
        if not (checkpoint_path / "config.json").is_file():
            continue
        if checkpoint_path == run_dir:
            selections.append(
                CheckpointSelection(
        ...
        if checkpoint_path.parent != run_dir:
            continue
        match = STEP_CHECKPOINT_RE.match(checkpoint_path.name)
        if match:
            selections.append(
                CheckpointSelection(
                    label=_checkpoint_label(checkpoint_path, run_dir),
...
def load_transformer(
    ...
    model_vocab_size = int(adapter.model.get_input_embeddings().num_embeddings)
    tokenizer_vocab_size = len(tokenizer)
    if tokenizer_vocab_size != model_vocab_size:
        raise ValueError(
            "Tokenizer/model vocabulary mismatch: "
            f"{tokenizer_cfg} builds {tokenizer_vocab_size} tokens, but "
            f"{checkpoint} expects {model_vocab_size}. Use the tokenizer config "
            "from the training run, for example configs/data/14.yaml for the "
            "matched tok14 Transformer checkpoints."
        )
    _MODEL_CACHE = LoadedTransformer(
        checkpoint_path=checkpoint,
...
def generate_music(
    ...
    output_path = out_dir / f"transformer-generated-{timestamp}.mid"
    try:
...
def create_demo(args: argparse.Namespace):
    ...
    with gr.Blocks(title="Transformer MIDI Generator") as demo:
        gr.Markdown("# Transformer MIDI Generator")
        with gr.Row():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch a Gradio UI for Transformer MIDI generation.")
